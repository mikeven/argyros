<?php
	...
	function obtenerRegistroOrdenPorId( $dbh, $ido, $idu ){
		...
		o.admin_note, o.revision_note, o.client_buy_note as nota_compra_cliente, o.order_status as estado, 
	...
	function registrarOrden( $dbh, $orden ){
		...
		$q = "insert into orders ( user_id, client_buy_note, total_price, total_count, created_at, 
		order_read, order_status ) values ( $orden[id_user], '$orden[nota_cliente]', $orden[total_pedido], 
		$orden[total_items], NOW(), '', 'pendiente' )";
		
		$data = mysqli_query( $dbh, $q );
		return mysqli_insert_id( $dbh );
	}
	/* ----------------------------------------------------------------------------------- */
	function obtenerDatosCreacionOrden( $carrito, $nota ){
